...
class PfpManagerTest extends DbTestCase
{
    ...
    public function testGetAllPfp()
    {
        $pfpManager = \VOCApp::getInstance()->getService('pfp');
        $pfps = $pfpManager->getAllPfp();

        $this->assertTrue($pfps[0] instanceof Pfp);
        $this->assertEquals(count($pfps), 5);
    }

    public function testGetWeightNumberFromPfpDescription()
    {
       $pfpId = 5;
       $pfp = new Pfp();
       $pfp->setId($pfpId);
       $pfp->load();
       $pfpManager = \VOCApp::getInstance()->getService('pfp');
       $number = $pfpManager->getWeightNumberFromPfpDescription($pfp);
       $this->assertEquals($number, "750");
    }
    
    public function testGetWeightLetterFromPfpDescription()
    {
        $pfpId = 5;
        $pfp = new Pfp();
        $pfp->setId($pfpId);
        $pfp->load();
        $pfpManager = \VOCApp::getInstance()->getService('pfp');
        $letter = $pfpManager->getWeightLetterFromPfpDescription($pfp);
        $this->assertEquals($letter, "/ EC");
    }
