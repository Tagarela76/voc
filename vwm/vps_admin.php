...
switch ($_GET['vpsAction']) {
	...
	case "browseCategory":
		...
		switch ($itemID) {
			...
			case "customers":
				$vpsCustomerManager = new VPSCustomerManager($db);
				$customers = $vpsCustomerManager->getCustomerList();
		
				$smarty->assign("customers", $customers);
				$smarty->assign("registeredCount", $vpsCustomerManager->registeredCustomersCount);
				$smarty->assign("unRegisteredCount", $vpsCustomerManager->unRegisteredCustomersCount);
	...
	case "viewDetails":
		...
		switch ($itemID) {
			...
			case "customer":
				...
				$customer = new VPSCustomer($db, $customerID);

				$customerInvoices = new VPSCustomerInvoices($db, $customer);
				$trialInvoice = $customerInvoices->getTrialInvoice();
				$currentInvoice = $customerInvoices->getCurrentInvoice();
				...
				$smarty->assign('trialInvoice', $trialInvoice);
				$smarty->assign('currentInvoice', $currentInvoice);
				$smarty->assign('invoices', $allInvoices);
				$smarty->assign('currency', $customer->getCurrency());
			...
			case "notRegisteredCustomer":
				...
				$trialEndDate->add( new DateInterval("P1M") );

				$smarty->assign('customer', $customer);
				$smarty->assign('trialPeriodEnd', $trialEndDate);
...
//							}
			...
			case "notRegisteredCustomer":
				$xnyo->filter_post_var('customerID', 'int');
				$xnyo->filter_post_var('what2edit', 'text');
				$xnyo->filter_post_var('trial_period_end', 'text');
				$customerID = $_POST['customerID'];
				$what2edit = $_POST['what2edit'];
				$trialPeriodEnd = $_POST['trial_period_end'];

				switch ($what2edit) {
					case 'trialPeriodEnd':
						break;

					case 'setupFirstPeriod':
						$billingManager = new VPSBillingManager($db, new VPSCurrency($db, 1));
						...
						$smarty->assign("months", $months);
						$smarty->assign("sources", $sources);
						$smarty->assign("billingPlans", $billingPlans);
						$smarty->assign("bookmarkType", "selectBillingPlan");
						$smarty->assign("newUserRegistration", true);
						$smarty->assign("trial_period_end", $trialPeriodEnd);
						$smarty->assign("customerID", $customerID);
						$smarty->display("tpls:vps.tpl");
						break;
...
/*** Edit billing plan ***/
			case "customer":
				...
				$customer = new VPSCustomer($db,$customerID);
				$invoiceManager = new VPSCustomerInvoices($db, $customer);
				$currency = new VPSCurrency($db, $customer->currency_id);
				...
				switch ($what2edit) {
					case "billing":
						...
						$selectedBilling = new VPSBilling($db, $_POST['billingID'], $currency);
						...
						if ($nextInvoice){var_dump($customer->balance);
							if($nextInvoice->status == VPSInvoice::STATUS_PAID){
								$customer->balance = $customer->balance + $nextInvoice->total;
							}							
							$nextInvoice->status = VPSInvoice::STATUS_CANCELED;
							$nextInvoice->save();

							//var_dump($invoiceManager->getNextPeriodInvoice());die();
						}
						//	and now let's insert invoice for paid period
						$invoiceItem = new VPSInvoiceItem($db);
						$invoiceItem->setup4paidPeriod($selectedBilling);

						$invoice = new VPSInvoice($db);
						...
						$customer->next_billing_id = $newBillingPlan;
						$customer->save();
...
die();	
			...
			case "notRegisteredCustomer":
				...
				$xnyo->filter_post_var('what2edit', 'text');
				$xnyo->filter_post_var('trial_period_end', 'text');
				$xnyo->filter_post_var('selectedBillingPlan', 'text');
				$customerID = $_POST['customerID'];
				$what2edit = $_POST['what2edit'];

				$trialEndDate = DateTime::createFromFormat(DEFAULT_DATE_FORMAT, $_POST['trial_period_end']);				
				if (!$trialEndDate) {
					throw new Exception('Could not parse Trial Period End Date - '.html_entity_encode($_POST['trial_period_end']));
				}
				...
				switch ($what2edit) {
					...
					case 'register':
						$db->beginTransaction();
						$currency = new VPSCurrency($db, DEFAULT_CURRENCY);

						$customer = new VPSCustomer($db, $customerID);
						if ($customer->getBilling()) {
							throw new Exception("Customer {$customerID} is already registered");
						}

						$selectedBilling = new VPSBilling($db, $_POST['selectedBillingPlan'], $currency);

						$customer->billing_id = VPSBilling::TRIAL_PERIOD_ID;
						$customer->next_billing_id = $selectedBilling->billing_id;
						$customer->status = VPSCustomer::STATUS_ON;
						$customer->discount = 0;
						$customer->balance = 0;
						...
						//	also we need to create invoice to simplify billing period tracking
						$invoiceItem = new VPSInvoiceItem($db);
						$invoiceItem->setup4trialPeriod();

						$trialInvoice = new VPSInvoice($db);
						$trialInvoice->setCustomer($customer);
						$trialInvoice->addItem($invoiceItem);
						$trialInvoice->period_end_date = clone $trialEndDate;
						$trialInvoice->suspension_date = clone $trialEndDate;
						$trialInvoice->period_start_date = clone $trialInvoice->generation_date;
						$trialInvoice->status = VPSInvoice::STATUS_DUE;
						$trialInvoice->suspension_disable = true;
						$trialInvoice->currency_id = $customer->currency_id;
						$trialInvoice->save();
						...
						$invoice->setCustomer($customer);
						$invoice->addItem($invoiceItem);
						$invoice->period_start_date = clone $trialInvoice->period_end_date;
						$invoice->period_start_date->add(new DateInterval("P1D"));
						$invoice->period_end_date = clone $invoice->period_start_date;
						$invoice->period_end_date->add(new DateInterval("P{$selectedBilling->months_count}M"));
						$invoice->suspension_date = clone $invoice->period_start_date;
	...
	case "ping":
		...
		$rows = $db->fetch_all();
		foreach ($rows as $row) {
			$sql = "UPDATE company SET trial_end_date = ";
		}
		break;
}
?>
