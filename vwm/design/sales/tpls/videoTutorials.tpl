...
<table cellspacing="0" cellpadding="0" align="center" colls="2" style="border-collapse: collapse;">
	<tr>
		...
		<td style="vertical-align: bottom;" rowspan="2">
			<a href="../videoTutorial/{$category}/training{$category|capitalize}.mp4" style="display:block;width:720px;height:480px; border: 1px solid black;" id="player"></a>
	...
	<tr>
		<td align="center" width="160px" style="vertical-align: bottom;padding-right: 20px;">
			...
			<b id="caption">{$category|capitalize} Level Tutorial</b><br/><br/>
			</td></tr></table>
						...
						<td align="center" id="{$key}" style="padding: 10px; margin: 0; border: 1px solid black;">
							<a style="text-decoration: none;" onClick="changeColor('{$key}');$f('player', '../videoTutorial/flowplayer.commercial-3.2.7.swf',
							   {literal}
								...
								clip: {
							   {/literal}
									url: '../videoTutorial/{$category}/{$key}{$category|capitalize}.mp4',
							   {literal}
									autoPlay: false,
									autoBuffering: true
								}
	...
	<input type="hidden" id="hiddenPart" value="{$category}"/>
</tr>
{if $category == "company"}
<tr>
	<td>
...
{literal}
	<script>
		...
		function createHtml(level){
			...
			var levelF = ['How to Login', 'Overview', 'Create Report', 'Facility at a Glance Graphs', 'How to Manage SDS & Product Library', 'How to Add a New Product', 'Equipment Management', 'Equipment Graphs', 'Payment Process', 'See Entire Video','New Product Video'];	
			var	levelD = ['How to Login', 'Overview', 'Pre Formaulated Mix', 'Single Mix Input', 'Create Report', 'How to Manage SDS & Product Library', 'How to Add a New Product', 'Equipment Management', 'Equipment Graphs', 'See Entire Video','New Product Video'];
			var result = '';
			switch (level){
				case 'company':
					levelAll = levelC;
