			...
			<td  class="">
				<a style='color:white;' onclick='$("#sort").attr("value","{if $sort==5}6{else}5{/if}"); $("#sortForm").submit();'>
		            <div style='width:100%;  color:white;'>					
						GOM ID {if $sort==5 || $sort==6}<img src="{if $sort==5}images/asc2.gif{/if}{if $sort==6}images/desc2.gif{/if}" alt=""/>{/if}
					</div>
				</a>
			</td>
			<td  class="">
				<a style='color:white;' onclick='$("#sort").attr("value","{if $sort==1}2{else}1{/if}"); $("#sortForm").submit();'>
		            <div style='width:100%;  color:white;'>					
						GOM Name {if $sort==1 || $sort==2}<img src="{if $sort==1}images/asc2.gif{/if}{if $sort==2}images/desc2.gif{/if}" alt=""/>{/if}
					</div>
				</a>
			</td>
			<td  class="">
				...
				<a href="{$product->url}{if $page}&page={$page}{/if}{if $request.subBookmark}&subBookmark={$request.subBookmark}{/if}" style="text-decoration: none;"><div style="width:100%;">
					{foreach from=$comapnyList item=comapnyArr}	
						{foreach from=$comapnyArr item=comapny}	
							{if $product->product_id == $comapny.id}{$comapny.name}. {/if}
						{/foreach}		
					{/foreach}
					</div ></a>
			</td>			
...
{*BEGIN	EMPTY LIST*}
		<tr class="">
		    <td  class="border_users_l border_users_r" style='text-align:center; vertical-align:middle;' colspan="5" >No accessories</td>
		</tr>
{*END	EMPTY LIST*}
