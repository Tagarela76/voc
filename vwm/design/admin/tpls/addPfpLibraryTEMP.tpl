						...
						<tr>
							...
							<td class="border_users_r border_users_b">
								...
								{if $validStatus.products eq 'noProducts'}
									{*ERORR*}										
										<div class="error_img"><span class="error_text">No products in the PFP!</span></div>
									{*/ERORR*}
								{/if}
								{/if}
...
{*ADDPRODUCTS*}
	...
	<div align="right" class="margin5">
		<input type='button' name='cancel' class="button" value='Cancel' 
				{if $request.action eq "addItem"} onClick="location.href='admin.php?action=browseCategory&category=pfps&bookmark={$request.category}&subBookmark={$request.subBookmark}{if $request.letterpage }&letterpage={$request.letterpage}{/if}{if $request.page }&productCategory={$request.productCategory}{/if}'"
				{elseif $request.action eq "edit"} onClick="location.href='?action=viewDetails&category=pfpLibrary&bookmark={$request.category}&subBookmark={$request.subBookmark}&id={$request.id}{if $request.letterpage }&letterpage={$request.letterpage}{/if}{if $request.page }&productCategory={$request.productCategory}{/if}'"
				{/if}
		>
