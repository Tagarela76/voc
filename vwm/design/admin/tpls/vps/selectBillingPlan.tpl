<form action="?action=vps&vpsAction=confirmEdit&itemID=notRegisteredCustomer" method="post">
		...
		<table border="0"  width="100%"  cellspacing="0" cellpadding="5" >
			...
			{section name=i loop=$sources}
				<tr class="billingPlans">
					...
					{foreach from=$billingPlans item=billingPlan}
						{if $sources[i] eq $billingPlan->bplimit}
							{assign var="currency" value=$billingPlan->getCurrency()}
							<td class="billingPlans_bot">
								<input type="radio"
										name="selectedBillingPlan"
										onClick="billingRadioButtonClick('{$availablePlans[j].type}');"  value="{$billingPlan->billing_id}">
										{$currency->sign} {$billingPlan->price}
							</td>
						{/if}
					{/foreach}
				</tr>
			...
			<tr class="billingPlans">
				...
				{assign var="sourcesCount" value=$sources|@count}
				<td  class="billingPlans_bot" colspan="{$sourcesCount*2+1}" align="center">If you want to use 4 and more emission sources, you should
...
<table border="0"  width="100%"  cellspacing="0" cellpadding="5" >
		...
		<div style='text-align:right;width:100%;'>
			<input name="what2edit" type="hidden" value="register"/>
			<input name="trial_period_end" type="hidden" value="{$trial_period_end}"/>
