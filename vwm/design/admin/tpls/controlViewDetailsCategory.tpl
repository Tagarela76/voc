...
{if $request.action != ""}
	...
	{else}
	<input type="button" class="button" value="Delete" onclick="location.href='admin.php?action=deleteItem&category={$request.category}{if $request.bookmark}&bookmark={$request.bookmark}{/if}&itemsCount=1&id[]={$request.id}'">	
	{/if}
{/if}
</div>
</div></div></div>