{if $request.bookmark == "docs" || $request.bookmark == "reminder" || $request.bookmark == "reminderUsers"}	
 ...
 <div>
    ...
    <a href="?action=browseCategory&category={$request.category}&id={$request.id}&bookmark=reminderUsers&tab=reminderUsers" {if $request.bookmark == "reminderUsers"} class="active_link" {/if}>Reminders Users</a>
 </div>	
{/if}