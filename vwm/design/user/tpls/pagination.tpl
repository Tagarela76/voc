...
<tr>
	<td align="center" class="pagination">	
		{if $currentPage != 1 && $currentPage}
			<a href="{$currentURL}&sortBy={$sortBy}&page={$currentPage-1}{if $sort}&sort={$sort}{/if}" class="NextPrevious">Previous</a>					
		{/if}
		...
		{if $smarty.section.i.index+1 >= $first && $smarty.section.i.index+1 <= $last}
			...
			{else}
				<a href="{$currentURL}&sortBy={$sortBy}&page={$smarty.section.i.index+1}{if $sort}&sort={$sort}{/if}">{$smarty.section.i.index+1}</a>
			{/if}
		{/if}			
		{/section}		
		
		{if $currentPage != $pageCount && $pageCount != 0}			
			<a href="{$currentURL}&sortBy={$sortBy}&page={$currentPage+1}{if $sort}&sort={$sort}{/if}" class="NextPrevious">Next</a>
		{/if}		
	</td>	
</tr>
</table>
