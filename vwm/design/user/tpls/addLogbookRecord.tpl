...
<form action="" method="post">
    <div class="padd7">
        ...
        {if $inspectionTypesList}
            ...
            itlManager.inspectionTypeList.changeLogbookDescriptionList()">
    ...
    <tr class="border_users_b border_users_r" height='30'>
        ...
        <td>
            <input type='checkbox' id='isRecurring' name='isRecurring' onchange="itlManager.showLogbookPeriodicity()" {if $logbook->getIsRecurring()}checked='true'{/if}>
        </td>
    </tr>
    <tr class="border_users_b border_users_r" height='30' hidden="hidden" id='periodicityContainer'>
        ...
        <td>
            <select name='periodicity' id='periodicity'>
                {foreach from=$logbookPeriodicityList item='logbookPeriodicity'}
                    <option value='{$logbookPeriodicity.id}' {if $logbook->getPeriodicity() == $logbookPeriodicity.id}selected='true'{/if}>
                        {$logbookPeriodicity.description|escape}
                    </option>
                {/foreach}
            </select>
        </td>
