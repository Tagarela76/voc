...
$nr = date("t",mktime(12,0,0,$month,1,$year));
        ...
        echo "<a href=calendar.php?op=cal&month=".$pm."&year=".$py.">  <= ".$maand[$pm]." - ".$py."</a>"; }
        echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$askedmonth." ".$askedyear."";
...
echo "<table cellspacing=0 >";
    ...
    echo "</tr>";
        ...
        for ($i=1;$i<=$nr;$i++){
            ...
            $query = "select id,title,description from events left join calendar_cat on events.cat=calendar_cat.cat_id where day='$i' and month='$month' and year='$year' and approved='1' order by day,month,year ASC";
			$result = mysql_query($query);
			...
			// ADD EVENT ON CALENDAR CLICK
			echo "onclick=\"MM_openBrWindow('calendar.php?op=eventform&day={$i}&month={$month}&year={$year}','Calendar','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=".$popupeventwidth.",height=".$popupeventheight."')\"";
            ...
            while ($row = mysql_fetch_object($result)){
				...
				echo "<span class='title'>";
					if ($popupevent == 1){
						echo "<a href=\"#\" onclick=\"MM_openBrWindow('cal_popup.php?op=view&id=".$row->id."','Calendar','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=".$popupeventwidth.",height=".$popupeventheight."')\">".stripslashes($row->title)."</a>";
					}else{
						echo "<a href=calendar.php?op=view&id=".$row->id.">".stripslashes($row->title)."</a>";
					}
				echo"</span>";	
					echo "<span class=\"desc\">".stripslashes($row->description)."</span>";
				echo "</li>";
            }
			if(mysql_num_rows($result)){
				echo "</ul></div>";
