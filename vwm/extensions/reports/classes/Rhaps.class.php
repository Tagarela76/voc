...
class Rhaps extends ReportCreator implements iReportCreator
{
    ...
    const TB_MIX = 'mix';
    const TB_MIX_GROUP ='mixgroup';
    const TB_COMPONENTS_GROUP=' components_group';
    const TB_COMPONENTS = 'component';
    const TB_DEPARTMENT = 'department';
    const TB_FACILITY = 'facility';

    function Rhaps($db, ReportRequest $reportRequest)
    {
        $this->db = $db;
        ...
        switch ($this->categoryType) {
            case "company":
                 $query = "SELECT p.product_nr, c.cas, c.description, ".
                         ...
                         "FROM ".self::TB_PRODUCT." p ".
                         "LEFT JOIN ".self::TB_MIX_GROUP." mg ".
                         "ON p.product_id=mg.product_id ".
                         "LEFT JOIN ".  self::TB_MIX." m ".
                         "ON m.mix_id=mg.mix_id ".
                         "LEFT JOIN ".self::TB_COMPONENTS_GROUP." cg ".
                         "ON p.product_id = cg.product_id ".
                         "LEFT JOIN ".self::TB_COMPONENTS." c ".
                         "ON cg.component_id = c.component_id ".
                         "LEFT JOIN ".self::TB_DEPARTMENT." d ".
                         "ON d.department_id = m.department_id ".
                         "LEFT JOIN ".self::TB_FACILITY." f ".
            ...
            case "department":
				...
				//var_dump($query);die();
				$facilityIDS = array();
                $facility = new VWM\Hierarchy\Facility($this->db, $department->getFacilityId());
