...
class XML2PDF extends PDF_MC_Table
{
	...
	var $curYear; //current year&rule to write in total
	var $curRule;
	...
	function startElement($path) 
		...
		switch ($tag) 
		{
			...
			case "RULE":
				...
				$this->Cell(190,10,"Rule: ".$name,0,0,'L');
    	   		$this->Ln(5);
    	   		$this->curRule = $name;
				break;
			...
			case "YEAR":	
				...
				$this->Cell($this->widths[1],7,"Total VOC ",1,0,'C');
				$this->Ln();
				$this->curYear = $name;
				break;
			...
			case "INFO":	
			$this->SetFont('Arial','',12);
				$this->rows[0] = $attribs["MONTH"];
				$this->rows[1] = $attribs["VOC"];
				break;
		...
		switch ($tag) 
		 {			 			
			...
			case "TOTAL":
				...
				$this->Cell($this->widths[0],7,"Total ".$this->curYear." by rule ".$this->curRule,1,0,'L',true);
