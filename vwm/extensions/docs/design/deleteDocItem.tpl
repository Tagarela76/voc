...
<div class="padd7">
	<form name="deleteDocItem" enctype="multipart/form-data" action=
		{if $step eq 'choose'}
			"?action={$request.action}&category=docs&facilityID={$request.facilityID}"
		{elseif $empty neq 'true'} 
			"?action=confirmDelete&category=docs&facilityID={$request.facilityID}"
		{else}
			"?action=browseCategory&category=facility&id={$request.facilityID}&bookmark=docs"
		{/if} method="POST">
		...
		<div align="right">
			<input type='button' name='cancel' class="button" value='Cancel' 
				onClick="location.href='?action=browseCategory&category=facility&id={$request.facilityID}&bookmark=docs'">
			{if $step eq 'choose'}	
