...
//		$timeStampTrialDaysLeft = strtotime($customer['trial_end_date']) - strtotime(date('Y-m-d'));
//
//		if ( $timeStampTrialDaysLeft < $configs['vps_registration_period']*24*60*60 && $timeStampTrialDaysLeft > 0) {
//			$daysLeft = (int)round($timeStampTrialDaysLeft/60/60/24);
//		} else {
//			$safeCustomerID = (int)$customer['id'];					
//			$query = "SELECT MIN(DATEDIFF(i.suspension_date, '".$currentDate."')) days_left " .
//    			 					"FROM ".TB_VPS_INVOICE." i, ".TB_VPS_CUSTOMER." c " .
//    			 					"WHERE i.customer_id = c.customer_id AND i.suspension_disable=1 AND i.status = 'due' AND c.customer_id=".$safeCustomerID;
//			$db->query($query);
//			if ($db->num_rows() > 0) {					
//				$data = $db->fetch(0);
//				$daysLeft = (!is_null($data->days_left)) ? (int)$data->days_left : "NULL";					
//			}
//		}
//			
//	//	$bridge->setCustomerDeadLineCounter($customer['id'], $daysLeft); //no need in it - now calculated automatically
	}		
