...
//		//selecting customers who should pay
//		$query = "SELECT i.invoice_id, i.customer_id, CURDATE() today,i.period_end_date, DATEDIFF(i.period_end_date, CURDATE()) days_left " .
//			"FROM ".TB_VPS_INVOICE." i, ".TB_VPS_CUSTOMER." c " .
//			"WHERE DATEDIFF(i.period_end_date, CURDATE()) < ".$config['invoice_generation_period']." " .
//			"AND c.status = 'on' " .
//			"AND i.billing_info IS NOT NULL " .
//			"AND i.customer_id = c.customer_id";
////		$query = "SELECT i.invoice_id, i.customer_id, CURDATE() today, i.suspension_date, DATEDIFF(i.suspension_date, CURDATE()) days_left " .
////			"FROM ".TB_VPS_INVOICE." i, ".TB_VPS_CUSTOMER." c " .
////			"WHERE DATEDIFF(i.suspension_date, CURDATE()) < ".$config['invoice_generation_period']." " .
////			"AND c.status = 'on' " .
////			"AND i.billing_info IS NOT NULL " .
////			"AND i.customer_id = c.customer_id";
//		$db->query($query);
...
//				);				
//				
//				//filter customers: keep customers only with old invoices
//				$query = "SELECT * " .
//					"FROM ".TB_VPS_INVOICE." " .
//					"WHERE customer_id = ".$invoice['customerID']." " .
//					"AND period_start_date >= '".$invoice['period_end_date']."' " .
//					"AND billing_info IS NOT NULL " .
//					"AND status <> 'canceled'";
//				$db->query($query);
//				
//				if (!$db->num_rows()) {	// here they are!					
//					$log .= createInvoice($invoice, $db, $config);					
//				} else {
//					
//					//	if invoice already created, but not paid then email notification
//					$invoiceObj = new Invoice($db);
...
//					if ($invoiceDetails['status'] == "DUE") {						
//						//A-ha! Customer didn't pay! Ok, let's notify him.
//						$log .= manageCustomersAndNotify($invoice["customerID"], $invoice['daysLeft'], $config, $db);						
//					}
//					
//				}	//	end if !$db->num_rows()
//			}															
//		}
    ...
    foreach ($customers as $customer)
    {
		...
		//$bridge->setCustomerDeadLineCounter($customerID, $daysLeft);		
