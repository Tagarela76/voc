package {
	...
	public class uploader extends Sprite {
		...
		private var domain:String = "vocwebmanager.co.uk";
...
//			}
			...
			// check caller. Allow only from domain
...
//			if(domainCheck.test(url)) {
				...
				inifileRefListener();
//			} else {
//				ExternalInterface.call("debug", url);
//			}
		}
		...
		private function upload(event:MouseEvent):void {
			...
			for (var i:Number = 0; i < totalList.length; i++) {
				...
				request.url="http://"+domain+"/dev/vwm/index.php";				
		...
		private function checkLimits():void {
			...
			xmlLoader.load(new URLRequest("http://"+domain+"/dev/bridge/bridge.xml?"+Math.random()));
		}
