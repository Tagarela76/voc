...
class validateCSV {
	...
	public function validate($input) {
		...
		while ($dat = fgetcsv($file, 1000, ";")) {
			...
			$data_tmp[26] = $data[$headerKey['industryType']];
			$data_tmp[27] = $data[$headerKey['industrySubType']];
			...
			if ($inProduct){
				if (!empty($data[26])) {
					...
					*/
					$industryType = array(
						"industryType" => $data[26],
						"industrySubType" => $data[27]
					);
					$product["industryType"][] = $industryType;
					$industryTypeEnd = FALSE;
				} else if (empty ($data[27])){
			...
			if (!empty($data[11]) && $inProduct) {
				...
				if ($industryTypeEnd) {
				$tmpArray = array(6, 7, 10, 11, 12, 13, 14, 25);
				$column = "";
	...
	private function tableHeader($file) {
		...
		$possibleIndustryType = array('INDUSTRY TYPE');
		$possibleIndustrySubType = array('INDUSTRY SUB-CATEGORIES');
		...
		$columnsArray = array('productID', 'mfg', 'industryType', 'industrySubType', 'productName', 'type', 'scoating', 'aerosol', 'substrate',
