...
class CAUsers extends Controller
{
    ...
    private function actionAddItem()
    {
        if ($_POST['save'] == 'Register') {
            ...
            if ($this->user->isValidRegData($data, $check)) { //echo "HERE USER SAVED WITH NEXT DATA:";var_dump($data);die();						
                $userID = $this->user->addUser($data);
                
                //add reminder User if we in facility level
                if (isset($data['facility_id'])) {
                    $reminderUser = new ReminderUser();
                    $reminderUser->setUserId($userID);
                    $reminderUser->setFacilityId($data['facility_id']);
                    $reminderUser->setEmail($data['email']);
    ...
    private function actionEdit()
    {
        ...
        if ($_POST['save'] == 'Save') {
            ...
            if ($this->user->isValidRegData($data, $check)) {
                ...
                $reminderUser = $rManager->getReminderUserByUserId($data['user_id']);
