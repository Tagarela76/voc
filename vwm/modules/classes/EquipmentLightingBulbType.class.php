...
class EquipmentLightingBulbType {

	/**
	 *
	 * @var int 
	 */
	public $equipment_lighting_bulb_type_id;
	...
	function __construct(db $db, $equipment_lighting_bulb_type_id = null) {
		$this->db = $db;
		
		if (isset($equipment_lighting_bulb_type_id)) {
			$this->equipment_lighting_bulb_type_id = $equipment_lighting_bulb_type_id;
			$this->_load();
		}
	}
	...
	public function save() {
		...
		$query = "INSERT INTO " . TB_EQUIPMENT_LIGHTING_BULB_TYPE . "(name) VALUES ('" . $this->db->sqltext($this->name) . "')";
		$this->db->query($query);
	...
	public function delete() {
		...
		$sql = "DELETE FROM ". TB_EQUIPMENT_LIGHTING_BULB_TYPE. "
				 WHERE equipment_lighting_bulb_type_id=" . $this->db->sqltext($this->equipment_lighting_bulb_type_id); 
		$this->db->query($sql);
	}
	...
	private function _load() {
		...
		if (!isset($this->equipment_lighting_bulb_type_id)) {
			return false;
		}
		$sql = "SELECT * 
				FROM ". TB_EQUIPMENT_LIGHTING_BULB_TYPE. "
				 WHERE equipment_lighting_bulb_type_id=" . $this->db->sqltext($this->equipment_lighting_bulb_type_id) . 
				" LIMIT 1"; 
		$this->db->query($sql);
