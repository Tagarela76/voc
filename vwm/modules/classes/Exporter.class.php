<?php
	 ...
	 * $exporter->export();
	 * 	
	 * @author Denis Tukalenko <denis.nt@kttsoft.com>	 
	 *
	 */
	...
	class Exporter {
		...
		public function export() {
			...
			if ($this->_buildXML($filePath)) {				
				...
				//	Ксю, здесь будет созадаваться pdf
		...
		private function _buildXML($filePath) {
			...
			$body = $doc->createElement( "body" );
				...
				//Я СКРЫЛ поиск и фильтры ВРЕМЕННО. Д
				// 	<ADDITIONAL>
//				$additional = $doc->createElement( "additional" );
//					
//					// search
//					$search_term = $doc->createElement( "search_term" );
//					$search_term->appendChild( $doc->createTextNode( self::protectString($this->search_term) ) );
//					$additional->appendChild($search_term);
//					
//					//	filter
//					$filter = $doc->createElement( "filter" );
//						// each filter item is a property of this class, PLS carefully
//						$filterItems = array('field', 'condition', 'value');
//						foreach ($filterItems as $filterItem) {
//							if (!property_exists($this, $filterItem)) {
//								//	no such property
//								if (self::DEBUG) {
//									echo "Undefined property at Exporter Class:<i>'".$filterItem."'</i><br>";
//								}
//								$this->error = true;
//								return false;
//							}
//							$item = $doc->createElement( $filterItem );
//							$item->appendChild( $doc->createTextNode( self::protectString($this->$filterItem) ) );
//							$filter->appendChild( $item );
//						}														
//					$additional->appendChild($filter);
//					
//				$body->appendChild($additional);
				// </ADDITIONAL>
