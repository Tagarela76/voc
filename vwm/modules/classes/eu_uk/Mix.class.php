...
class Mix extends MixProperties {
	...
	public function addNewMix($usageData, $isMWS = false) {
		...
		$query = "INSERT INTO ".TB_USAGE." (equipment_id, department_id, description, voc, voclx, vocwx, creation_time, rule_id,apmethod_id, exempt_rule, waste_percent ) VALUES (".			
			$query .= "'".$usageData['equipment_id']."', ";
			$query .= "'".$usageData['department_id']."', ";
	...
	public function getMixDetails($usageID, $vanilla = false, $isMWS = false) {
		...
		$query = "SELECT * FROM ".TB_USAGE." WHERE mix_id=".$usageID;
		$this->db->query($query);
...
//			}
	...
	public function getWasteDetails($mixID = 0) {								
		...
		if (!$wasteFromDB) {			
			...
			//	get company ID for VOC unit type
			$query = "SELECT f.company_id FROM ".TB_FACILITY." f, ".TB_DEPARTMENT." d, ".TB_USAGE." m " .
					"WHERE f.facility_id = d.facility_id AND " .
					"d.department_id = m.department_id AND " .
					"m.mix_id = ".$mixID."";
			$this->db->query($query);
			
			$company = new Company($this->db);
			$companyDetails = $company->getCompanyDetails($this->db->fetch(0)->company_id);
			$vocUnittype = $companyDetails['voc_unittype_id']; 
			
			$unittype = new Unittype($this->db);			
			$unitTypeConverter = new UnitTypeConverter($unittype->getDescriptionByID($vocUnittype));
			...
			$wasteDetails['calculated'] = $this->calculateWaste($wasteDetails['unittypeID'], $wasteDetails['value'], $unitTypeConverter, $quantitySum);	//	waste in weight unit type same as VOC		
		}		
	...
	public function countMixes($departmentID, $filter) {
	...
	public function mixAutocomplete($occurrence, $departmentID) {
		...
		$query = "SELECT mix_id, description, LOCATE('".$occurrence."', description) occurrence FROM ".TB_USAGE." WHERE department_id = ".$departmentID;
		$this->db->query($query);
...
/**	 
	...
	//$unittypeID	- input unittype
	//$value		- input value
	//$unitTypeConverter - with initialized To default
	//$quantitySum = - product sum qty
	private function calculateWaste($unittypeID, $value, $unitTypeConverter, $quantitySum = 0, $mixDensity = false) {
		...
		//	for ($i = 0; $i < count($products); $i++) {
			foreach($products as $data) {
				$product = array (
					...
					"unittype"		=>	$data/*$products[$i]*/->unit_type								
				);								
			//	$query = "SELECT * FROM ".TB_PRODUCT." WHERE product_id=".$products[$i]->product_id;
			//	$this->db->query($query);
			//	$data = $this->db->fetch(0);
				$product["product_nr"] = $data->product_nr;
				$product["description"] = $data->name;
				$product["voclx"] = $data->voclx;
				$product["vocwx"] = $data->vocwx;	
				$product["density"] = $data->density;
				$product["densityUnitID"] = $data->density_unit_id;
