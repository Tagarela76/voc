...
//	   		$vps2voc->setCustomerLimits($customerID, $billingPlanDetails['limits']);
    ...
    public function getBillingPlanDetails($billingID, $customerID = false, $currencyID = 1) {
		...
		//default limit
			...
			$billingPlan['limits'] = $this->getDefaultLimit($billingID,$currencyID);
		}
     ...
     */
    public function getModuleBillingPlans($moduleBillingPlanID = null,$currencyID = 1) {
    	...
    	else if(is_array($moduleBillingPlanID))
    	{
			...
			for($i = 0; $i < $len; $i++) // Hard check for array of numbers.
			{
    ...
    public function removeModuleBillingPlan($customerID,$moduleBillingPlanID) {
		...
		//	check is this module part of multyinvoice
    	//	if yes - create new multy invoice without module
    	//	if no - just cancel invoice
