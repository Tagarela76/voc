...
class calendar_plugin
{
    ...
    {
        $this_year = $this->dateNow("%Y");
        $this_month = $this->dateNow("%m");
        $this_day = $this->dateNow("%d");
    ...
    function weekOfYear($day,$month,$year)
    {
        ...
        $week_number = ceil(($this->julianDate($day,$month,$year)
    ...
    function beginOfNextMonth($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        return $this->dateFormat($day,$month,$year,$format);
    ...
    function nextWeekday($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $days = $this->dateToDays($day,$month,$year);

        if ($this->dayOfWeek($day,$month,$year) == 5) {
            $days += 3;
        } elseif ($this->dayOfWeek($day,$month,$year) == 6) {
            $days += 2;
        } else {
            $days += 1;
        }
    ...
    function prevWeekday($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        if ($this->dayOfWeek($day,$month,$year) == 1) {
            $days -= 3;
        } elseif ($this->dayOfWeek($day,$month,$year) == 0) { 
            $days -= 2;
        } else {
            $days -= 1;
        }
    ...
    function nextDayOfWeek($dow,$day='',$month="",$year="",$format="%Y%m%d",$onOrAfter=false)
    {
        ...
        $days = $this->dateToDays($day,$month,$year);
        $curr_weekday = $this->dayOfWeek($day,$month,$year);
    ...
    {
        return($this->nextDayOfWeek($dow,$day="",$month="",$year="",$format="%Y%m%d",true));
    ...
    {
        return($this->prevDayOfWeek($dow,$day="",$month="",$year="",$format="%Y%m%d",true));
    ...
    {
        if (!$this->isValidDate($day1,$month1,$year1)) {
            return -1;
        }
        if (!$this->isValidDate($day2,$month2,$year2)) {
            return -1;
        }

        return(abs(($this->dateToDays($day1,$month1,$year1))
                    - ($this->dateToDays($day2,$month2,$year2))));
    } // end func dateDiff
    ...
    */
    ...
    {
        $ndays1 = $this->dateToDays($day1, $month1, $year1);
        $ndays2 = $this->dateToDays($day2, $month2, $year2);
        if ($ndays1 == $ndays2) {
            return 0;
        }
    ...
    function weeksInMonth($month="",$year="")
    {
        ...
        if (DATE_CALC_BEGIN_WEEKDAY == 1) {
            if ($this->firstOfMonthWeekday($month,$year) == 0) {
                $first_week_days = 1;
            } else {
                $first_week_days =
                    7 - ($this->firstOfMonthWeekday($month,$year) - 1);
            }
        } else {
            $first_week_days = 7 - $this->firstOfMonthWeekday($month,$year);
        }

        return ceil((($this->daysInMonth($month,$year) - $first_week_days) / 7) + 1);
    ...
    function firstOfMonthWeekday($month="",$year="")
    {
        ...
        return($this->dayOfWeek("01",$month,$year));
    ...
    function beginOfMonth($month="",$year="",$format="%Y%m%d")
    {
        ...
        return($this->dateFormat("01",$month,$year,$format));
    ...
    function beginOfWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $this_weekday = $this->dayOfWeek($day,$month,$year);

        if (DATE_CALC_BEGIN_WEEKDAY == 1) {
            if ($this_weekday == 0) {
                $beginOfWeek = $this->dateToDays($day,$month,$year) - 6;
            } else {
                $beginOfWeek = $this->dateToDays($day,$month,$year)
                    - $this_weekday + 1;
            }
        } else {
                $beginOfWeek = ($this->dateToDays($day,$month,$year)
       ...
       /*  $beginOfWeek = ($this->dateToDays($day,$month,$year)
        ...
        return($this->daysToDate($beginOfWeek,$format));
    ...
    function endOfWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $last_dayOfWeek = ($this->dateToDays($day,$month,$year)
            + (6 - $this_weekday + DATE_CALC_BEGIN_WEEKDAY));

        return($this->daysToDate($last_dayOfWeek,$format));
    } // end func endOfWeek
    ...
    function beginOfNextWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $date = $this->daysToDate($this->dateToDays($day+7,$month,$year),"%Y%m%d");
        ...
        $this_weekday =
            $this->dayOfWeek($next_week_day,$next_week_month,$next_week_year);

        $beginOfWeek = ($this->dateToDays($next_week_day,$next_week_month,$next_week_year)
            - ($this_weekday - DATE_CALC_BEGIN_WEEKDAY));
    ...
    function getCalendarWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $curr_day = $this->beginOfWeek($day,$month,$year,"%E");

        for($counter=0; $counter <= 6; $counter++) {
            $week_array[$counter] = $this->daysToDate($curr_day,$format);
            $curr_day++;
        }
    ...
    function getCalendarMonth($month="",$year="",$format="%Y%m%d")
    {
        ...
        if (DATE_CALC_BEGIN_WEEKDAY == 1) {
            if ($this->firstOfMonthWeekday($month,$year) == 0) {
                $curr_day = $this->dateToDays("01",$month,$year) - 6;
            } else {
                $curr_day = $this->dateToDays("01",$month,$year)
                    - $this->firstOfMonthWeekday($month,$year) + 1;
            }
        } else {
            $curr_day = ($this->dateToDays("01",$month,$year)
                - $this->firstOfMonthWeekday($month,$year));
        }
        ...
        for($row_counter=0; $row_counter < $weeksInMonth; $row_counter++) {
            for($column_counter=0; $column_counter <= 6; $column_counter++) {
                $month_array[$row_counter][$column_counter] =
                    $this->daysToDate($curr_day,$format);
                $curr_day++;
            }
        }
    ...
    function getCalendarYear($year="",$format="%Y%m%d")
    {
        ...
        for($curr_month=0; $curr_month <=11; $curr_month++) {
            $year_array[$curr_month] =
                $this->getCalendarMonth(sprintf("%02d",$curr_month+1),$year,$format);
        }
    ...
    function daysToDate($days,$format="%Y%m%d")
    {
        ...
        return($this->dateFormat($day,$month,$century.$year,$format));
    } // end func daysToDate
    ...
    function NWeekdayOfMonth($occurance,$dayOfWeek,$month,$year,$format="%Y%m%d")
    {
        ...
        $DOW1 = $this->dayOfWeek($DOW1day,$month,$year);

        $wdate = ($occurance - 1) * 7 + 1 +
                (7 + $dayOfWeek - $DOW1) % 7;

        if ( $wdate > $this->daysInMonth($month,$year)) {
            return -1;
        } else {
            return($this->dateFormat($wdate,$month,$year,$format));
        }
    ...
    function dateFormat($day,$month,$year,$format)
    {
        ...
        for($strpos = 0; $strpos < strlen($format); $strpos++) {
            ...
            if ($char == "%") {
                ...
                switch($nextchar) {
                    case "a":
                        $output .= $this->getWeekdayAbbrname($day,$month,$year);
                        break;
                    case "A":
                        $output .= $this->getWeekdayFullname($day,$month,$year);
                        break;
                    case "b":
                        $output .= $this->getMonthAbbrname($month);
                        break;
                    case "B":
                        $output .= $this->getMonthFullname($month);
                        break;
                    case "d":
                    ...
                    case "e":
                        ...
                        break;
                    case "E":
                        $output .= $this->dateToDays($day,$month,$year);
                        break;
                    case "j":
                        $output .= $this->julianDate($day,$month,$year);
                        break;
                    case "m":
                    ...
                    case "t":
                        ...
                        break;
                    case "w":
                        $output .= $this->dayOfWeek($day,$month,$year);
                        break;
                    case "U":
                        $output .= $this->weekOfYear($day,$month,$year);
                        break;
                    case "y":
    ...
    function getMonthAbbrname($month,$length=3)
    {
        ...
        return substr($this->getMonthFullname($month), 0, $length);
    ...
    function getWeekdayFullname($day="",$month="",$year="")
    {
        ...
        $weekday_names = $this->getWeekDays();
        $weekday = $this->dayOfWeek($day,$month,$year);
    ...
    function getWeekdayAbbrname($day="",$month="",$year="",$length=3)
    {
        ...
        return substr($this->getWeekdayFullname($day,$month,$year),0,$length);
    } // end func getWeekdayFullname
